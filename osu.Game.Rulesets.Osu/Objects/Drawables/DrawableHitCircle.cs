...
namespace osu.Game.Rulesets.Osu.Objects.Drawables
{
    public partial class DrawableHitCircle : DrawableOsuHitObject, IHasApproachCircle
    {
        ...
        public partial class HitReceptor : CompositeDrawable, IKeyBindingHandler<OsuAction>
        {
            ...
            {
                base.Update();
                // Увеличиваем хитбокс через BigHitbox из ModMenu
                // Визуальный размер кружка не меняется — только зона нажатия
                float mult = OsuModMenuBridge.BigHitboxEnabled ? BIG_HITBOX_MULTIPLIER : 1.0f;
                Size = OsuHitObject.OBJECT_DIMENSIONS * mult;
                CornerRadius = OsuHitObject.OBJECT_RADIUS * mult;
            }
